            ...
            <div class="wrap" style="position: relative;">
                ...
                <table class="wp-table widefat fixed striped posts">
                    <thead>
                        <tr>
                            <th>
                                <a href="<?php echo admin_url('admin.php?page=loan_application');?>" class=""><i class="dashicons dashicons-arrow-left-alt"></i>Go Back</a>
                            </th>
                ...
                <table class="wp-table widefat fixed striped posts">
                    ...
                    <?php foreach ($single_form as $data) : ?>
                        ...
                        <tr>
                            <td>Type of Loan: </td>
                            <td><?php echo $data->type_of_loan ?></td>
                        </tr>
                        <tr>
                            <td>Loan Amount: </td>
...
echo "<span style='color: #ff0000; font-weight: bold;'>Not Eligible</span>";
                            ...
                            <td><a href="<?php echo admin_url('admin.php?page=loan_application&view=results&id='.$data->id) ?>" class='button button-primary'>Details</a></td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
        ...
        <div id="tab2" class="tab-pane">
            <?php
                $table = $wpdb->prefix.'loancard_contact'; 
                $sql = "SELECT * FROM $table WHERE status='done' AND type='loan' ORDER BY id DESC";
...
echo "<span style='color: #ff0000; font-weight: bold;'>Not Eligible</span>";
        ...
        <div id="tab3" class="tab-pane">
            <?php 
                $sql = "SELECT * FROM $table WHERE status='pending' AND type='loan' ORDER BY id DESC";
                // $sql = "SELECT * FROM $table";
                $results = $wpdb->get_results($sql);
                if (count($results) <= 0) {
                    echo "Data not found!";
...
echo "<span style='color: #ff0000; font-weight: bold;'>Not Eligible</span>";
        ...
        <div id="tab4" class="tab-pane">
            <?php 
                $sql = "SELECT * FROM $table WHERE status='not_eligible'AND type='loan' ORDER BY id DESC";
                // $sql = "SELECT * FROM $table";
                $results = $wpdb->get_results($sql);
                if (count($results) <= 0) {
                    echo "Data not found!";
